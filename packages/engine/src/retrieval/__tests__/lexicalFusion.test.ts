...
import { computeIdf, fuseRRF } from '../lexicalFusion';
...
describe('fuseRRF', () => {
    ...
    // Parity lock for the Map-based rank lookup (Monorepo WO-04): an id listed
    // twice in one ranking must score by its FIRST occurrence — the semantics
    // the old indexOf implementation had implicitly.
    it('duplicate ids within one ranking score by first occurrence', () => {
        const fused = fuseRRF(['a', 'b', 'a', 'c'], ['c', 'b']);
        expect(fused[0]).toBe('b'); // kw rank 1 + emb rank 1 beats a (kw 0 only) and c (kw 3 + emb 0)
        expect(fused).toEqual(['b', 'c', 'a']);
    });
});
