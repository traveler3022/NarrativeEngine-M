...
export const api = {
    ...
    campaigns: {
        async saveState(campaignId: string, state: { context: GameContext; messages: ChatMessage[]; condenser: CondenserState }): Promise<void> {
            const { saveCampaignState } = await import('../services/persistence/campaignStore');  // @rf RF-004 W0 — domain→state, switch to CampaignContextPort
            await saveCampaignState(campaignId, state);
        },
        async saveNPCs(campaignId: string, npcs: NPCEntry[]): Promise<void> {
            const { saveNPCLedger } = await import('../services/persistence/campaignStore');  // @rf RF-004 W0 — domain→state, switch to CampaignContextPort
