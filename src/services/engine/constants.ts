/**
 * Engine defaults — neutral source of truth for world/encounter/surprise
 * generation tables.
 *
 * ── Why this lives here, not in settingsSlice ───────────────────────────
 * These arrays are *data* the engine consumes. They are not settings:
 * they don't get persisted, they don't change at runtime, they have no
 * UI affordance. They were originally exported from settingsSlice for
 * historical reasons (the slice happened to use them to seed the
 * default GameContext). That colocation made services that needed the
 * tables (engineRolls, campaignSlice) import from the store layer — a
 * boundary leak.
 *
 * Hoisting them to a neutral engine module breaks that leak without
 * changing behavior. settingsSlice now re-exports them for backward
 * compatibility with any caller we missed, but the canonical home is
 * here.
 */
