/**
 * @refactor RF-004, RF-006, RF-013
 * @violations 2 (see architecture/reverse-engineering/0.15-architecture-violations/RAW_DATA.json)
 * @waves W0(advance)/W1(close); W0(advance)/W2(close); W9
 * @ports CampaignContextPort, NotificationPort, (God File split)
 * @godFile RF-013 (1238 lines)
...
import { loadChapters } from '../../services/persistence/campaignStore';  // @rf RF-004 W0 — domain→state, switch to CampaignContextPort
        ...
        try {
            ...
            if (sealProvider && tierAllows(state.settings.aiTier, 'sealChapter')) {
                ...
                if (sealResult.divergences.length > 0 && liveRegister && callbacks.setDivergenceRegister) {
                    ...
                    notificationPort.warning('Chapter sealed but divergence facts failed to parse');
                }
            ...
            notificationPort.success('Chapter sealed');
        } catch (err) {
            console.error('[SealChapter] Failed to seal chapter:', err);
            notificationPort.error('Failed to seal chapter');
        }
    }
}
