/**
 * @refactor RF-010, RF-014, RF-007
 * @violations 3 (see architecture/reverse-engineering/0.15-architecture-violations/RAW_DATA.json)
 * @waves W6; W10; W0(advance)/W3(close)
 * @ports (logic extraction), (slice split), NotificationPort
 * @godFile RF-014 (614 lines)
...
import { EMPTY_REGISTER, toggleChapter, toggleCategory, pinFact, editFact, deleteFact, deleteChapter, toggleFact, dismissReviewFlag, editKnownBy, applySubjectTokens, migrateV1ToV2 } from '../../utils/divergenceUtils';
import { debouncedSaveCampaignState } from '../../services/persistence/campaignStateSave';
import { uid } from '../../utils/uid';
import { deleteMessageImage, deleteAllCampaignImages, countTextTokens } from '../../services/chatLifecycle';
...
export const createChatSlice: StateCreator<ChatDeps, [], [], ChatSlice> = (set) => ({
    ...
    deleteMessage: (id) =>
        set((s) => {
            ...
            if (s.activeCampaignId) deleteMessageImage(s.activeCampaignId, id);
            debouncedSaveCampaignState(s.activeCampaignId, { context: s.context, messages: msgs, condenser: s.condenser, pinnedExcerpts: s.pinnedExcerpts });
            return { messages: msgs };
        }),
        ...
        set((s) => {
            ...
            if (s.activeCampaignId) {
                for (const m of removed) {
                    deleteMessageImage(s.activeCampaignId, m.id);
                }
            }
    ...
    clearChat: () => set((s) => {
        ...
        if (s.activeCampaignId) deleteAllCampaignImages(s.activeCampaignId);
    ...
    addPinnedExcerpt: (sourceMessageId, text, isFullMessage) => {
        ...
        set((s) => {
            const newTokens = countTextTokens(text);
            const currentTotal = s.pinnedExcerpts.reduce((sum, e) => sum + countTextTokens(e.text), 0);
